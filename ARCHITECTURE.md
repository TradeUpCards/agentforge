...
| §5.2 (eval) | **26 cases across 6 categories** (`happy_path`, `auth_boundary`, `edge_case`, `ambiguous`, `prompt_injection`, `leakage_attempt`), not the 8 originally listed. Two-mode eval (fixture mode for CI determinism + live LLM for realism). 12 synthetic patient fixtures in the `999100-999114` sentinel range, each engineered for one specific failure mode (sparse data, polypharmacy DDI, free-text-heavy, pediatric, contradictory progression, 4 prompt-injection variants, leakage lure). LLM-as-judge in original §5.2 not built — explicit non-goal for week 1. Pre-commit hook runs the smoke tier on every commit. See [`EVAL_SUITE.md`](./EVAL_SUITE.md) + [`agent/tests/eval/COVERAGE.md`](./agent/tests/eval/COVERAGE.md) for the full picture. |
...
| §8.2 (CI/CD) | **Pre-commit hook + GitHub Actions PR gate + GitLab CI mirror — all shipped 2026-05-02.** `scripts/git-hooks/pre-commit` runs the smoke tier on every commit (~4s, no LLM cost). [`.github/workflows/agent-eval.yml`](./.github/workflows/agent-eval.yml) re-runs verifier + mask unit tests + eval Golden Set on every PR + push to master/agentforge/* — defense-in-depth against pre-commit bypass. [`.github/workflows/agent-deploy.yml`](./.github/workflows/agent-deploy.yml) is a manual-trigger deploy stub awaiting SSH-secret wiring. [`.gitlab-ci.yml`](./.gitlab-ci.yml) mirrors the GitHub workflow as `when: manual` (labs.gauntletai.com instance has no Runner provisioned for student projects; same logic auto-runs on the GitHub mirror). ([`.github/workflows/README.md`](./.github/workflows/README.md)) |
| §10 (roadmap) | Week-2 candidate backlog inventoried in local notes — broader than the original §10 list, with provenance + size estimates per candidate. Cross-patient pre-fetch explicitly *rejected*; same-patient drawer-open pre-warm queued as week-2 work. Per-finding "week 3" tags in `AUDIT.md` carry the production-hardening backlog. |
...
| (new — security, 2026-05-02) | **HMAC replay protection** via signed timestamp in the request payload. Agent rejects requests >30s off its clock (in either direction). Closes the ai-security-review production blocker #3. ([DECISIONS.md revision log](./DECISIONS.md)) |
| (new — security, 2026-05-02) | **Per-user rate limiting + hourly token budget** middleware in `agent/main.py` keyed on `user_id`. Defaults: 60 RPM + 200K tokens/hr/user. Closes the ai-security-review production blocker #1. ([`agent/_rate_limit.py`](./agent/_rate_limit.py)) |
| (new — observability, 2026-05-02) | **Langfuse sessions + users + per-LLM-call latency** + **PHI date-bucketing mask** + **outbound PHI scrubber Tier 2**. Sessions group multi-turn UC3 conversations; users power per-user spend rollups; mask scrubs day-precision dates + SSN/phone/email/MRN from observability traces; outbound scrubber refuses responses containing cross-patient `patient_id`/SSN/phone/email/MRN before they reach the user. Cross-patient name detection deferred ([DECISIONS.md §4a "Why name detection is deferred"](./DECISIONS.md#4a-phi-redaction-implementation-plan)). ([`agent/_phi_scrubber.py`](./agent/_phi_scrubber.py)) |
| (new — audit, 2026-05-02) | **`agent_log` PHI audit table** — every `/chat` request writes one row with request_id, user/patient/use_case, prompt, tools called, LLM telemetry, verifier verdict, claim counts, response, latency, outcome. Closes [AUDIT.md C-1](./AUDIT.md) (HIPAA §164.312(b) gap that EventAuditLogger doesn't auto-log SELECTs). Dedicated `agent_audit_rw` DB user with INSERT-only privileges. ([`agent/_audit_log.py`](./agent/_audit_log.py)) |
| (new — operations, 2026-05-02) | **[`SLO.md`](./SLO.md) + [`RUNBOOK.md`](./RUNBOOK.md)** added. SLO doc names 5 SLOs (availability 99.5%/30d, verifier pass rate ≥95%/7d, citation match rate ≥85%/7d, latency P95 <8s, tool failure rate <1%/7d) with thresholds + page-vs-ticket triage. RUNBOOK names backup cadence, three restore scenarios with RTO targets, monthly drill cadence. Alert routing + backup automation deferred as named operational follow-up. Closes sys-arch-review production blockers #2 + #3. |
| (new — workflow, 2026-05-02) | **[`WORKFLOW.md`](./WORKFLOW.md)** added — git workflow + dual-mirror sync rules. Repo lives on GitLab `origin` (primary) + GitHub mirror; `origin` configured with two push URLs. Documents the merge-on-GitLab-only-then-mirror pattern that prevents the dual-merge SHA divergence trap that bit us early in the day on `agentforge/ci-cd-workflows`. |
...
> **Updated 2026-05-02.** Implementation is **26 cases across 6 categories** (`happy_path`, `auth_boundary`, `edge_case`, `ambiguous`, `prompt_injection`, `leakage_attempt`) — not the 8 categories listed below. **Three tiers** (smoke / full / nightly) gate which cases run in pre-commit vs CI vs manual; fixture mode for determinism + live mode against Synthea-imported patients for realism. **12 synthetic patient fixtures** in the `999100-999114` sentinel range purpose-built for specific failure modes. **GitHub Actions + GitLab CI** wired to re-run the smoke + full tiers on every PR. **LLM-as-judge not built** — explicit non-goal for week 1; deterministic asserts cover the brief's named failure modes. See [`EVAL_SUITE.md`](./EVAL_SUITE.md) and [`agent/tests/eval/COVERAGE.md`](./agent/tests/eval/COVERAGE.md) for the full current picture, including the gaps (Replay Harness, scale).
...
> **Updated 2026-05-02.** Several "production hardening" items originally listed here shipped during week 1: per-user rate limiting + token budget, `agent_log` PHI audit table, HMAC replay protection, GitHub Actions + GitLab CI gates, [`SLO.md`](./SLO.md) (5 SLOs + alerting plan), [`RUNBOOK.md`](./RUNBOOK.md) (backup/restore procedures), outbound PHI scrubber Tier 2 (cross-patient ID + SSN/phone/email/MRN; name detection deferred). What remains for week 3 is below.

- Production hardening: full Safe Harbor 18-identifier sweep on inbound retrieved-records (DECISIONS.md §4a), proper secrets manager (currently env vars), external audit-log forwarding (ATNA syslog), anomalous-access alerting (SLO routing layer wiring per SLO.md §4)
