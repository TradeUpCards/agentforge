...
9. **Cache hit rate measurement needs re-baseline against Synthea-shaped contexts.** The n=50 baseline observed 0% cache reads, but the test setup used Maria fixture (~500 tokens of prefix) which is below Anthropic's ~1024-token prompt-cache size threshold — caching is documented to no-op below that size. `agent/tests/verify_cache.py` against patient_id=92 (Synthea-Guadalupe, ~6,953 tokens) shows cache_read=10,057 on the second of two identical calls (100% of the cached prefix served). Cache works; the baseline measured the wrong shape. The eval suite is well-positioned to gate cache reads once we add a UC3-shaped case against a Synthea patient — a multi-turn case should produce ≥85% cache_read on turns 2+. Filed as a P2 follow-up in [`SLO.md §3`](../../../SLO.md). Re-baseline cost: ~30min of compute, ~$0.30 of Anthropic spend.
